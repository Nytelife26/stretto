...
use crate::utils::{
	change_lifetime_const, SharedValue, ValueRef, ValueRefMut,
};
...
type Shards<V, SS> =
	Box<[RwLock<HashMap<u64, StoreItem<V>, SS>>; NUM_OF_SHARDS]>;
...
impl<V: Send + Sync + 'static> ShardedMap<V> {
	...
	pub fn new() -> Self {
		Self::with_validator(
			ExpirationMap::new(),
			DefaultUpdateValidator::default(),
		)
	}
}
...
impl<V: Send + Sync + 'static, U: UpdateValidator<V>> ShardedMap<V, U> {
	...
	pub fn with_validator(
		...
		Self { shards, em, store_item_size: size, validator }
	}
}
...
{
	pub fn with_validator_and_hasher(
		em: ExpirationMap<ES>,
		validator: U,
		hasher: SS,
	) -> Self {
	...
	fn cleanup_inner<PS: BuildHasher + Clone + 'static>(
		&self,
		policy: Policy<PS>,
	) -> Vec<CrateItem<V>> {
		let now = Time::now();
		self.em.cleanup(now).map_or(Vec::with_capacity(0), |m| {
			m.iter()
				// Verify that the store agrees that this key is expired.
				.filter_map(|(k, v)| {
	...
	pub fn cleanup<PS: BuildHasher + Clone + 'static>(
		...
		self.cleanup_inner(Policy::Sync(policy))
	}
	...
	pub fn cleanup_async<PS: BuildHasher + Clone + 'static>(
		...
		self.cleanup_inner(Policy::Async(policy))
	}
...
enum Policy<PS: BuildHasher + Clone + 'static> {
	#[cfg(feature = "sync")]
	Sync(Arc<LFUPolicy<PS>>),
	#[cfg(feature = "async")]
	Async(Arc<AsyncLFUPolicy<PS>>),
}

impl<PS: BuildHasher + Clone + 'static> Policy<PS> {
	pub fn cost(&self, k: &u64) -> i64 {
		match self {
			#[cfg(feature = "sync")]
			Policy::Sync(policy) => policy.cost(k),
			#[cfg(feature = "async")]
			Policy::Async(policy) => policy.cost(k),
		}
	}
	...
	pub fn remove(&self, k: &u64) {
		match self {
			#[cfg(feature = "sync")]
			Policy::Sync(policy) => policy.remove(k),
			#[cfg(feature = "async")]
			Policy::Async(policy) => policy.remove(k),
		}
	}
}
